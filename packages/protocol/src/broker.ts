...
interface SharedFollow {
  ...
  /** Block hash -> local follow tokens and the replay snapshot still owning it. */
  pins: Map<string, Set<string>>;
}

// Local tokens are generated by the broker; this owner is never exposed on wire.
const SNAPSHOT_PIN = "snapshot";
...
class ChainBroker {
      ...
      if (request.id !== undefined) {
        this.sendToSession(
          ...
          method === "chainHead_v1_stopOperation"
            ? buildJsonRpcResult(request.id, null)
            : buildJsonRpcError(request.id, "Unknown subscription/token"),
        );
      }
      return;
    }
   ...
   * and forward upstream only when neither another session nor the current
   * replay snapshot holds it. Replies success (`null`) locally; a later
   * upstream release failure stops the shared generation for all followers.
   */
  ...
  private sendUpstreamUnpin(upstreamToken: string, hashes: string[]): void {
    const requestId = `broker-release:${this.requestCounter.toString(36)}`;
    this.requestCounter += 1;
    // Track the response: a failed release must not silently grow retention.
    this.pending.set(requestId, {
      sessionId: upstreamToken,
      clientId: null,
      method: "broker-unpin",
    });
    this.sendUpstream({
  ...
  private rewriteOwnedToken(
    ...
    if (!owned) {
      // chainHead operations must never forward a stale or foreign local token
      // verbatim. In particular, cancellation can race follow teardown.
      return (request.method as string).startsWith("chainHead_v1_")
        ? null
        : request;
    }
  ...
  private handleUpstreamResponse(response: JsonRpcResponse): void {
    ...
    if (pending.method === "broker-unpin") {
      const token = pending.sessionId;
      if ("error" in response && this.upstreamFollowTokens.has(token)) {
        this.handleUpstreamSubscription({
  ...
  private handleUpstreamSubscription(message: SubscriptionMessage): void {
    ...
    if (sharedFollow) {
      ...
      const isStop =
        isJsonRpcObject(eventResult) && eventResult.event === "stop";
      ...
      if (isStop) {
        // End the generation before delivery: onStop can synchronously re-follow.
        ...
        for (const localToken of sharedFollow.localTokens) {
          const local = this.localFollowTokens.get(localToken);
          if (local) {
            this.sessions.get(local.sessionId)?.ownedTokens.delete(localToken);
          }
          this.localFollowTokens.delete(localToken);
        }
        sharedFollow.pins.clear();
        for (const [id, request] of this.pending) {
          if (
            request.method === "broker-unpin" &&
            request.sessionId === upstreamToken
          ) {
      ...
      for (const { localToken, session } of recipients) {
        const eventType = isJsonRpcObject(eventResult)
          ...
          : "?";
        brokerLog(
          `← subscription [${session.id}] event=${eventType} method=${String(message.method)}`,
        );
        this.sendToSession(session, {
          ...message,
          params: {
  ...
  private cacheSharedFollowEvent(
    ...
    if (eventType === "initialized") {
      const hashes = Array.isArray(eventResult.finalizedBlockHashes)
        ...
        : [];
      const latest = hashes.at(-1);
      sharedFollow.finalizedBlockHashes = latest === undefined ? [] : [latest];
      if (latest !== undefined) {
        this.registerPin(sharedFollow, SNAPSHOT_PIN, latest);
      }
    ...
    if (eventType === "newBlock") {
      ...
      sharedFollow.blocks.set(blockHash, { ...eventResult });
      this.registerPin(sharedFollow, SNAPSHOT_PIN, blockHash);
      return;
    }
    ...
    if (eventType === "finalized") {
      const hashes = Array.isArray(eventResult.finalizedBlockHashes)
        ...
        : [];
      const previous = sharedFollow.finalizedBlockHashes;
      const latest = hashes.at(-1);
      for (const hash of hashes) {
        const runtime = sharedFollow.blocks.get(hash)?.newRuntime;
        if (runtime !== null && runtime !== undefined) {
          sharedFollow.finalizedBlockRuntime = runtime;
        }
        sharedFollow.blocks.delete(hash);
      }
      if (latest !== undefined) {
        sharedFollow.finalizedBlockHashes = [latest];
      }
      ...
      const obsolete = [...previous, ...hashes, ...pruned].filter(
        (hash) => !sharedFollow.finalizedBlockHashes.includes(hash),
      );
      const orphaned = this.releasePins(sharedFollow, SNAPSHOT_PIN, obsolete);
      if (orphaned.length > 0 && sharedFollow.upstreamToken !== null) {
        this.sendUpstreamUnpin(sharedFollow.upstreamToken, orphaned);
      }
    }
  ...
  private replayFollowSnapshot(
    ...
    // Map insertion order is parent-before-child by the follow contract.
    // Replay every current fork, not only best's ancestors: a later best change
    // may select any non-finalized block without repeating its newBlock event.
    for (const result of sharedFollow.blocks.values()) {
