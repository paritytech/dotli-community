...
import { hasSharedAuthSession } from "@dotli/protocol/client";
import {
  getCacheSettings,
  setCacheSettings,
  getBackend,
...
import { renderDiagnostics } from "./topbar-diagnostics";
...
function createPermissionDropdown(
  ...
  trigger.addEventListener("click", (e) => {
    ...
    const dropdownEvents = new EventManager();
    dropdownEvents.on(document, "click", onDocClick);
    dropdownEvents.on(document, "keydown", onKeyDown);

    openDropdownCleanup = (): void => {
      menu.remove();
      trigger.setAttribute("aria-expanded", "false");
      dropdownEvents.disposeAll();
    };
  });
...
function renderModePopover(): void {
  ...
  renderDiagnostics(rightCol, currentProductLabel);
