...
describe("PolkaVM host-frame response backpressure", () => {
  ...
  it("retries a deferred interruption before delivering the next one", () => {
    const interruptions = [Uint8Array.of(8), Uint8Array.of(9)];
    const { queue, posted, failures, runNextTimer } = harness({
      nextResponse: () => interruptions.shift(),
    });
    ...
    runNextTimer();
    expect(posted).toEqual([
      { bytes: Uint8Array.of(8), seq: 1 },
      { bytes: Uint8Array.of(8), seq: 2 },
    ]);
    queue.handleMessage({ type: "host-frame-response-accepted", seq: 2 });
    expect(posted.at(-1)).toEqual({ bytes: Uint8Array.of(9), seq: 3 });
    queue.handleMessage({ type: "host-frame-response-accepted", seq: 3 });
    expect(queue.pendingCount).toBe(0);
    expect(posted).toHaveLength(3);
    expect(failures).toEqual([]);
  ...
  it("retains a rejection racing with pause without spending the retry budget", () => {
    const { queue, posted, failures } = harness({ maxRetries: 0 });
    queue.start();
    ...
    expect(posted).toEqual([{ bytes: Uint8Array.of(1), seq: 1 }]);
    expect(failures).toEqual([]);
    queue.setPaused(false);
    expect(posted.at(-1)).toEqual({ bytes: Uint8Array.of(1), seq: 2 });
    queue.handleMessage({ type: "host-frame-response-accepted", seq: 2 });
    expect(posted.at(-1)).toEqual({ bytes: Uint8Array.of(2), seq: 3 });
    queue.handleMessage({ type: "host-frame-response-accepted", seq: 3 });
    expect(queue.pendingCount).toBe(0);
    expect(failures).toEqual([]);
  });

  it("suspends a scheduled retry and preserves responses until resume", () => {
    vi.useFakeTimers();
    try {
      const { queue, posted, failures } = harness({
      ...
      expect(posted).toEqual([{ bytes: Uint8Array.of(7), seq: 1 }]);
      expect(failures).toEqual([]);
      queue.setPaused(false);
      expect(posted.at(-1)).toEqual({ bytes: Uint8Array.of(7), seq: 2 });
      queue.handleMessage({ type: "host-frame-response-accepted", seq: 2 });
      expect(queue.pendingCount).toBe(0);
      queue.close();
    } finally {
