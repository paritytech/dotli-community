...
function disposeComputer() {
  const activeSupervisor = supervisor;
  supervisor = null;
  activeSupervisor?.dispose();
  pendingInput.length = 0;
  pendingInputBytes = 0;
  for (const socket of pendingNetworkPermissions.values()) {
    socket.close();
  }
  pendingNetworkPermissions.clear();
  translator = null;
...
function fail(error) {
  ...
  finished = true;
  try {
    if (supervisor !== null && initialized) {
      drainOutput();
    }
  } finally {
    try {
...
function flushPendingInput() {
  while (pendingInput.length > 0) {
    ...
    if (next.byteLength <= space) {
      ...
      pendingInputBytes -= next.byteLength;
      pendingInput.shift();
    } else {
      supervisor.sendTerminalInput(next.subarray(0, space));
      pendingInputBytes -= space;
    ...
    switch (message.type) {
      ...
      case "input": {
        if (supervisor !== null && !finished) {
          ...
          if (bytes.byteLength > MAX_PENDING_INPUT_BYTES - pendingInputBytes) {
            self.postMessage({
              type: "log",
              message: "terminal input dropped: pending input limit exceeded",
            });
            break;
          }
