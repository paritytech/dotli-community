...
import { archiveCar, type TestCar } from "./helpers/polkavm";

interface AppCar extends TestCar {
  manifest: string;
}

const fixtureRoot = join(import.meta.dirname, "fixtures/polkavm-computer");

async function appCar(
  ...
  const files = await Promise.all(
    entries.map(
      async ([name, path]) =>
        [
          name,
          new Uint8Array(await readFile(join(fixtureRoot, path))),
        ] as const,
    ),
  );
