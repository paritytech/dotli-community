 ...
 */
export function tryCreateLegacyIframeProvider(win, target, event) {
 ...
 */
function createLegacyHostProvider(win, target, frameOrigin) {
    const postOrigin = frameOrigin !== "" && frameOrigin !== "null" ? frameOrigin : "*";
    ...
    function onMessage(event) {
        if (closedError)
            return;
        if (event.source !== target || event.origin !== frameOrigin)
            return;
        if (!(event.data instanceof Uint8Array))
            return;
