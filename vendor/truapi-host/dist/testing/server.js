...
async function bundle(entry, wasmBundle = "testing") {
    ...
    const { build } = await import("esbuild").catch(() => {
        throw new Error("@parity/truapi-host/testing/server needs esbuild. Install it as a dev " +
 ...
 * ```
 */
export async function createTestHostServer(options = {}) {
    if (options.productAccounts) {
        throw new Error("createTestHostServer `productAccounts` is not supported by the TrUAPI " +
    ...
    const url = await new Promise((resolveUrl, reject) => {
        ...
        server.listen(options.port ?? 0, "127.0.0.1", () => {
            ...
            resolveUrl(options.productUrl
                ? hostPageUrl(base, options)
                : base);
        });
    });
    return {
