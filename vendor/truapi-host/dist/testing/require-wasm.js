...
export function wasmArtifact(relativePath) {
    return fileURLToPath(new URL(`../../dist/wasm/${relativePath}`, import.meta.url));
}
/**
 ...
 */
export function wasmIsBuilt(...relativePaths) {
    ...
    if (process.env.REQUIRE_WASM === "1") {
        throw new Error(`REQUIRE_WASM=1 but WASM artefacts are missing:\n  ${missing.join("\n  ")}\nRun \`npm run build:wasm\` first.`);
    }
    return false;
}
