...
function imageHeader(bytes, fileSize) {
...
function ebmlInteger(bytes, offset, id) {
...
function videoMime(bytes, fileSize) {
    ...
    if (!matches(bytes, 0, "\x1a\x45\xdf\xa3"))
        return undefined;
    const header = ebmlInteger(bytes, 4, false);
    if (!header || header.value > bytes.length - header.next)
        return undefined;
    const end = header.next + header.value;
    if (!matches(bytes, end, "\x18\x53\x80\x67"))
        return undefined;
...
export async function inspectNativeChatFileMetadata(blob, signal) {
...
export function nativeChatExportFilename(metadata) {
    let extension = "bin";
