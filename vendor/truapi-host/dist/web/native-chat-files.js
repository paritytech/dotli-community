...
import { inspectNativeChatFileMetadata, nativeChatExportFilename, } from "./native-chat-media.js";
...
export function createBrowserNativeChatFilesHost(sourceStore) {
    let disposed = false;
    let database;
    ...
    const host = {
            ...
            try {
                ...
                for (const file of picked) {
                    ...
                    const record = sourceStore
                        ...
                        : await store("readonly", (sources) => sources.get(file.sourceId));
        ...
        async readChatFile(sourceId, offset, length) {
            ...
            const record = sourceStore
                ...
                : await store("readonly", (sources) => sources.get(sourceId));
            if (!record || !(record.blob instanceof Blob))
            ...
            const bytes = new Uint8Array(await record.blob.slice(start, start + length).arrayBuffer());
