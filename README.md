...
Only builds compiled with `VITE_APP_DEBUG=true` offer the **Host inspector**. The debug bar shows the active full/Lite username, the username-check state, or **Connect wallet** when inactive; click it to open the inspector. It uses the debug panel's styling and stays below the header and above the debug footer, following resizing and docking without covering either. Docking reserves page width for the inspector, reflowing both the landing page and product content on wide desktops; phones use an overlay. Normal login continues to use Polkadot Mobile. Opening diagnostics with `?debug=true` or Settings in a production build does **not** enable wallet creation or restoration; existing experimental wallet storage is ignored and preserved.

Start with **Enable / use test wallet** and accept the warning to create or reuse a browser-local test identity. Username controls appear only after activation completes. An existing Lite username replaces the claim form; **Refresh username** verifies its on-chain ownership without submitting another registration. While active, the account badge also opens the Host inspector. Choose **Switch back to Mobile** in **Overview** to stop using the test wallet without deleting it. The normal login button then signs in with Polkadot Mobile. Mode changes reload the page to terminate the previous signing workers. The inspector tracks one active identity; it is not a multi-identity wallet manager.
...
**Claim Lite username** submits an explicitly confirmed registration on the selected network through the same-origin identity proxy. The inspector and debug-bar entry show the pending name while the native runtime automatically monitors chain ownership; closing the inspector does not cancel monitoring. An accepted claim is not resubmitted or reported as failed merely because confirmation is slow. Chain confirmation updates the claimed state and shows a notification, including while the inspector is closed. Disconnecting, replacing the wallet, or reloading the page ends that session's monitoring. **Refresh username** discovers an existing registration, including after importing a phrase in another profile; it is not needed to poll an in-progress claim. Unknown, last-known, confirmed-unclaimed, pending, confirmed-claimed, and failed states are distinguished. Confirmed public metadata is shared with other product hosts; live native sessions refresh in place without resetting their permissions. Cached names are rechecked on chain before being restored.

**Overview** shows the network, wallet identity, current product account public key and native derivation context, and product permissions. Allowance controls use the running product's native resource-allocation API and require an explicit request confirmation; native approval remains in charge. The API exposes allocation outcomes, not remaining quota, balances, amounts or fees. Results are labeled as last observed outcomes, and uncertain results are not retried automatically.

**Activity** shows timestamped, correlated operations for the current product and identity without messages, keys or arbitrary error payloads. It shares the debug capture's bounded history: Pause and Clear affect both views.

Under **Host inspector → Recovery**, **Reveal recovery phrase** requires confirmation before displaying a selectable phrase for manual backup. Existing 32-byte wallets export as 24 English BIP-39 words without changing their identity. Nothing is automatically copied or downloaded; hiding the phrase, leaving Recovery, closing the inspector or hiding the page clears it from the controls. Import and deletion controls are confined to Recovery.

**Import / replace test wallet** accepts checksum-valid English BIP-39 phrases of 12, 15, 18, 21 or 24 words, without a passphrase or custom derivation path. It uses native Polkadot host/Substrate derivation, not Bitcoin/Ethereum seed derivation. Importing an exported phrase restores the same account keys on the same network, but not permissions; use **Refresh username** to rediscover its registered name. Back up the previous test wallet before replacing it. Successful import replaces the shared wallet for trusted product hosts, clears wallet-bound experimental session/signing grants, activates the imported wallet and reloads open tabs; Mobile pairing and grants remain separate.
