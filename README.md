...
Only builds compiled with `VITE_APP_DEBUG=true` offer the **Host inspector**. The debug bar shows the active full/Lite username, the username-check state, or **Connect wallet** when inactive; click it to open the inspector. It uses the debug panel's styling and stays below the header and above the debug footer, following resizing and docking without covering either. It can dock beside the product on wide desktops and overlays only the product area on phones. Normal login continues to use Polkadot Mobile. Opening diagnostics with `?debug=true` or Settings in a production build does **not** enable wallet creation or restoration; existing experimental wallet storage is ignored and preserved.

Use **Enable / use test wallet** and accept the warning to create or reuse a browser-local test identity. While active, the account badge also opens the Host inspector. Choose **Switch back to Mobile** in **Overview** to stop using the test wallet without deleting it. The normal login button then signs in with Polkadot Mobile. Mode changes reload the page to terminate the previous signing workers. The inspector tracks one active identity; it is not a multi-identity wallet manager.
