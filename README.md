...
Only builds compiled with `VITE_APP_DEBUG=true` offer the **Host inspector**. The debug bar shows the active full/Lite username, **No username** when unregistered, or **Connect wallet** when inactive; click it to open the right-side drawer. It can dock beside the product on wide desktops and overlays the product on phones. Normal login continues to use Polkadot Mobile. Opening diagnostics with `?debug=true` or Settings in a production build does **not** enable wallet creation or restoration; existing experimental wallet storage is ignored and preserved.
...
**Claim Lite username** submits an explicitly confirmed registration on the selected network through the same-origin identity proxy. Success requires native chain ownership confirmation, not just backend acceptance. **Refresh username** discovers an existing registration, including after importing a phrase in another profile. Confirmed public metadata is shared with other product hosts; live native sessions refresh in place without resetting their permissions. Cached names are rechecked on chain before being restored.
